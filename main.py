...
# NAV BAR
# ------------------------
if 'auth_' not in st.session_state:  
    st.session_state['auth_'] = False
    
st.markdown("<h1 style='text-align: center; color:#9AE66E;'>CERTEROS 🎯</h1>", unsafe_allow_html=True)
place_holder_2 = st.empty()
divisor = place_holder_2.divider()


def authentication(username: str, password: str):
    db_users = get_users_auth()
    flag_auth = False
    for i in db_users:
    ...
    if flag_auth == True:
        return flag_auth,name,last_name,created_at
    else:
        ...
        with a2:
            ...
            if liga_choosed: # si la liga fue escogida
                ...
                if team_choosed:
                    ...
                    with col1:
                        st.markdown("<h3 style='text-align: center; color:#FFFFFF;'>Local 🏠</h3>", unsafe_allow_html=True)
                        st.write('---')
                    ...
                    with col2:
                        st.markdown("<h3 style='text-align: center; color:#FFFFFF;'>Visitante ⚠️</h3>", unsafe_allow_html=True)
                        st.write('---')
